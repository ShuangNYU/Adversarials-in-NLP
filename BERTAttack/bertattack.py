...
def get_substitues(substitutes, tokenizer, mlm_model):
    # substitues L,k
...
def attack(feature, tgt_model, mlm_model, tokenizer, k, batch_size, max_length=512, cos_mat=None, w2i={}, i2w={}):
    ...
    _, word_predictions = torch.topk(word_predictions, k, -1)  # seq-len k
    ...
    for top_index in list_of_index:
        ...
        substitutes = get_substitues(substitutes, tokenizer, mlm_model)
...
def evaluate(features):
    ...
    print('success-rate {:.6f}, query-num {:.4f}, change-rate {:.4f}'.format(suc, query, change_rate))
...
def run_attack():
    ...
    parser.add_argument("--start", type=int, help="5k /step, for multi-thread process")
    ...
    with torch.no_grad():
        for index, feature in enumerate(features[start:start+5000]):
            seq_a, label = feature
            ...
            feat = attack(feat, tgt_model, mlm_model, tokenizer_tgt, k, batch_size=32, max_length=512,
                          cos_mat=cos_mat, w2i=w2i, i2w=i2w)
